...
class AvsStyledTextCtrl(stc.StyledTextCtrl):
    ...
    def SetTextStyles(self, textstyles, monospaced=False):
        ...
        self.commentStyle = self.STC_AVS_COMMENT
        self.nonBraceStyles = [
    ...
    def setStylesNoColor(self):
        ...
        # Global default styles for all languages
        self.StyleSetSpec(stc.STC_STYLE_DEFAULT,"face:%(font)s,size:%(size)d,fore:%(color)s" % s_text)
        self.StyleClearAll()  # Reset all to be like the default
    ...
    def UpdateCalltip(self, force=False):
        ...
        boolIsComment = (self.commentStyle == self.GetStyleAt(caretPos - 1))
        ...
        while pos >= 0:
            ...
            if self.GetStyleAt(pos) not in (self.STC_AVS_STRING, self.STC_AVS_TRIPLE, self.STC_AVS_USERSLIDER):
                ...
                if c == '(':
                    nclose -= 1
            if nclose == 0:
                if self.GetStyleAt(pos) == self.commentStyle:
                    return None
                else:
                    return pos
            pos -= 1
    ...
    def OnKeyDown(self,event):
        ...
        if key in (wx.WXK_RETURN, wx.WXK_NUMPAD_ENTER, wx.WXK_TAB) and self.AutoCompActive():
...
class AvsStyleDialog(wx.Dialog):
    ...
    def __init__(self, parent, dlgInfo, options, title=_('AviSynth fonts and colors')):
        ...
        for tabLabel, tabInfo in dlgInfo:
            ...
            sizer = wx.FlexGridSizer(cols=4, hgap=20, vgap=10)
            sizer.Add((0,0), 0)
            ...
            for label, key in tabInfo:
                ...
                staticText = wx.StaticText(tabPanel, wx.ID_OK, label)
                if font is not None:
                    fontButton = wxButtons.GenButton(tabPanel, wx.ID_ANY, label=fontFace)
                ...
                if fontFore is not None:
                    ...
                    #~ foreButton.SetBackgroundColour(wx.Color(*fontFore))
                    #~ foreButton.SetCursor(wx.StockCursor(wx.CURSOR_HAND))
                    #~ foreButton.Bind(wx.EVT_LEFT_UP, self.OnButtonColor)
                    foreButton = colourselect.ColourSelect(tabPanel, wx.ID_ANY, colour=wx.Color(*fontFore), size=(50,23))
                else:
                    foreButton = None
                if fontBack is not None:
                    #~ backButton = wx.StaticText(tabPanel, wx.ID_ANY, size=(50, 20))
                    #~ backButton.SetBackgroundColour(wx.Color(*fontBack))
                    #~ backButton.SetCursor(wx.StockCursor(wx.CURSOR_HAND))
                    #~ backButton.Bind(wx.EVT_LEFT_UP, self.OnButtonColor)
                    backButton = colourselect.ColourSelect(tabPanel, wx.ID_ANY, colour=wx.Color(*fontBack), size=(50,23))
                else:
                    backButton = None
                sizer.Add(staticText, flag=wx.ALIGN_CENTER)
    ...
    def OnButtonFont(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            button.SetLabel(font.GetFaceName())
        button.SetFont(font)
        button.SetBestSize()
        button.Refresh()
        self.sizer.Fit(self)
...
class SliderPlus(wx.Panel):
    def __init__(self, parent, id, value=0, minValue=0, maxValue=100, size=(-1, 28)):
...
class MainFrame(wxp.Frame):
    ...
    def __init__(self, parent=None, id=wx.ID_ANY, title='AvsPmod', pos=wx.DefaultPosition, size=(700, 550), style=wx.DEFAULT_FRAME_STYLE):
        ...
        if self.options['startupsession']:
            if self.options['alwaysloadstartupsession'] or len(sys.argv) <= 1 or not self.options['promptexitsave']:
...
# The following code caused custom filter definitions to be deleted when the program exited. But, I have no idea what this code was actually supposed to do...
#
#        deleteKeys = []
#        for key, value in self.options['filteroverrides'].items():
#            if key.count('_') == 0:
#                tempList = [k for k in self.options['filteroverrides'] if k==key]
#                if len(tempList) == 1:
...
#            del self.options['filteroverrides'][key]
    ...
    def getOptionsDlgInfo(self):
        return (
            (_('General'),
                ...
                (_('External player:'), wxp.OPT_ELEM_FILE, 'externalplayer', _('Location of external program for script playback'), '*.exe'),
                (_('External player extra args:'), wxp.OPT_ELEM_STRING, 'externalplayerargs', _('Additional arguments when running the external player'), ''),
            ...
            (_('User Sliders'),
                ...
                (_('Create user sliders automatically'), wxp.OPT_ELEM_CHECK, 'autoslideron', _('Create user sliders automatically using the filter database'), ''),
                (_('type int/float (numerical slider)'), wxp.OPT_ELEM_CHECK, 'autoslidermakeintfloat', _('Create user sliders for int and float arguments'), 20),
                (_('type int (hex color)'), wxp.OPT_ELEM_CHECK, 'autoslidermakecolor', _('Create color pickers for hex color arguments'), 20),
                (_('type bool'), wxp.OPT_ELEM_CHECK, 'autoslidermakebool', _('Create radio boxes for bool arguments'), 20),
                (_('type string (list)'), wxp.OPT_ELEM_CHECK, 'autoslidermakestringlist', _('Create listboxes for string list arguments'), 20),
                (_('type string (filename)'), wxp.OPT_ELEM_CHECK, 'autoslidermakestringfilename', _('Create filename pickers for string filename arguments'), 20),
                (_('undocumented'), wxp.OPT_ELEM_CHECK, 'autoslidermakeunknown', _('Create placeholders for arguments which have no database information'), 20),
    ...
    def createWindowElements(self):
        ...
        #~ mdc.SetBackground(wx.Brush(wx.Color(90, 90, 90)))
        #~ mdc.Clear()
        #~ mdc = None
        ...
        menuBar = self.createMenuBar(self.menuInfo(), shortcutList, oldShortcuts)
        self.SetMenuBar(menuBar)
        scriptWindow.contextMenu = self.GetMenuBar().GetMenu(1)
        self.videoWindow.contextMenu = self.GetMenuBar().GetMenu(2)
        ...
        def OnProgramSplitterSize(event):
            ...
            if not self.separatevideowindow:
                ...
                self.videoSplitter.SetSize((self.programSplitter.GetSize()[0] - self.toggleSliderWindowButton.GetSize()[0], -1))
    ...
    def menuInfo(self):
        ...
        return (
            ...
            (_('&Edit'),
                ...
                (_('Unindent selection'), 'Shift+Delete', self.OnMenuEditUnIndentSelection, _('Unindent the selected lines')),
            ...
            (_('&Video'),
                ...
                (_('Zoom'),
                    ...
                    (reverseZoomLabelDict['100'], '', self.OnMenuVideoZoom, _('Zoom video preview to 100% (normal)'), wx.ITEM_RADIO, True),
                ...
                (_('Switch video/text focus'), 'Shift+Tab', self.OnMenuVideoSwitchMode, _('Switch focus between the video preview and the text editor')),
            ...
            (_('&Help'),
                ...
                (_('Open Avisynth plugins folder'), 'F1', self.OnMenuHelpAvisynthPlugins, _('Open the avisynth plugins folder')),
                (''),
    ...
    def createScriptWindow(self):
        ...
        scriptWindow.sliderWindow = wx.ScrolledWindow(self.videoSplitter, wx.ID_ANY, style=wx.STATIC_BORDER)
    ...
    def createVideoControls(self, parent, primary=True):
        ...
        if primary:
            ...
            self.videoSlider = SliderPlus(panel, wx.ID_ANY, 0, 0, 240-1)
            ...
            self.videoSlider2 = SliderPlus(panel, wx.ID_ANY, 0, 0, 240-1)
    ...
    def createTrimDialog(self, parent):
        ...
        choiceSizer = wx.BoxSizer(wx.HORIZONTAL)
        choiceSizer.Add(choiceLabel, 0, wx.ALIGN_CENTER_VERTICAL|wx.LEFT, 5)
        choiceSizer.Add(choiceBoxInsert, 1, wx.EXPAND|wx.LEFT, 5)
    ...
    def OnMenuFileRecentFile(self, event):
        ...
        try:
            ...
            if os.path.isfile(filename):
                ...
                wx.MessageBox(_('File does not exist!'), _('Error'), style=wx.ICON_ERROR)
        except IndexError:
            pass
    ...
    def OnMenuEditIndentSelection(self, event):
        script = self.currentScript
    ...
    def OnMenuVideoCropEditor(self, event):
        if self.zoomfactor != 1 or self.zoomwindow:
            wx.MessageBox(_('Cannot use crop editor unless zoom set to 100%!'), _('Error'), style=wx.ICON_ERROR)
            return False
        script = self.currentScript
        dlg = self.cropDialog
    ...
    def OnMenuMacrosFolder(self, event):
        if os.path.exists(self.macrofolder):
            ...
            wx.MessageBox(_('Could not find the macros folder!') % locals(), _('Error'), style=wx.ICON_ERROR)
    ...
    def OnMenuMacrosReadme(self, event):
        ...
        if os.path.exists(readme):
            ...
            wx.MessageBox(_('Could not find %(readme)s!') % locals(), _('Error'), style=wx.ICON_ERROR)
    ...
    def OnMenuToolsRunSelected(self, event):
        ...
        try:
            ...
            wx.MessageBox(_('Failed to import the selected tool'), _('Error'), style=wx.ICON_ERROR)
            return
    ...
    def OnMenuConfigureShortcuts(self, event):
        ...
        for window, idList in self._shortcutBindWindowDict.items():
            if window != self:
                exceptionIds += idList
        dlg = wxp.ShortcutsDialog(self, self.options['shortcuts'], exceptionIds=exceptionIds, submessage=_('* This shortcut is active only when video window has focus.'))
        ID = dlg.ShowModal()
        # Set the data
        if ID == wx.ID_OK:
            shortcutList = dlg.GetShortcutList()
    ...
    def OnMenuHelpAvisynthPlugins(self, event):
        ...
        if os.path.exists(plugindir):
            ...
            wx.MessageBox(_('Could not find the Avisynth plugins folder!') % locals(), _('Error'), style=wx.ICON_ERROR)
    ...
    def OnNotebookPageChanging(self, event):
        if self.cropDialog.IsShown():
            wx.MessageBox(_('Cannot switch tabs while crop editor is open!'), _('Error'), style=wx.ICON_ERROR)
            event.Veto()
        if self.trimDialog.IsShown():
            wx.MessageBox(_('Cannot switch tabs while trim editor is open!'), _('Error'), style=wx.ICON_ERROR)
            event.Veto()
        if self.FindFocus() == self.videoWindow:
    ...
    def OnMouseMotionVideoWindow(self, event):
        if self.cropDialog.IsShown() and event.m_leftDown:
            script = self.currentScript
            w = script.AVI.Width
            h = script.AVI.Height
            left = self.cropValues['left']
    ...
    def OnCropDialogSpinTextChange(self, event):
        script = self.currentScript
    ...
    def OnTrimDialogApply(self, event):
        ...
        if not self.InsertSelectionTrims(cutSelected=self.invertSelection, insertMode=insertMode):
    ...
    def OnConfigureVideoStatusBarMessage(self, event):
        ...
        noteText = wx.StaticText(dlg, wx.ID_ANY, _('Note: The "\\t\\t" is used to separate the left and right portions of the status bar message.'))
        sizer = wx.BoxSizer(wx.VERTICAL)
    ...
    def NewTab(self, copyselected=True, select=True, splits=None):
        if self.cropDialog.IsShown():
            wx.MessageBox(_('Cannot create a new tab while crop editor is open!'), _('Error'), style=wx.ICON_ERROR)
            return False
        if self.trimDialog.IsShown():
            wx.MessageBox(_('Cannot create a new tab while trim editor is open!'), _('Error'), style=wx.ICON_ERROR)
            return False
        self.Freeze()
    ...
    def SaveScript(self, filename='', index=None):
        ...
        if filename:
            ...
            if not os.path.isdir(dirname):
                wx.MessageBox(_('Directory %(dirname)s does not exist!') % locals(), _('Error'), style=wx.ICON_ERROR)
                return None
    ...
    def SaveCurrentImage(self, filename='', index=None):
        ...
        if filename:
            ...
            if script is None or script.AVI is None:
                wx.MessageBox(_('No image to save'), _('Error'), style=wx.ICON_ERROR)
                return False
            #~ if script==None:
                #~ script = self.currentScript
            w = script.AVI.Width
    ...
    def InsertSelectionTrims(self, cutSelected=True, insertMode=0):
        script = self.currentScript
    ...
    def _x_InsertBookmarkTrims(self):
        ...
        if nBookmarks <= 0:
            wx.MessageBox(_('No bookmarks defined!'), _('Error'), style=wx.ICON_ERROR)
            return
        ...
        if nBookmarks == 1:
            wx.MessageBox(_('There must be more than one unique bookmark to use this feature!'), _('Error'), style=wx.ICON_ERROR)
            return
    ...
    def DeleteFrameBookmark(self, value, bmtype=0, refreshVideo=True):
            ...
            try:
                ...
                for slider in sliderList:
                    if bmtype != bmtype2:
                        ...
                        #~ self.DeleteFrameBookmark(value, bmtype, refresh=refreshProgram)
                        self.DeleteFrameBookmark(value, bmtype)
            except ValueError:
    ...
    def SetVideoCropStatusText(self):
        ...
        if ar == '1.778:1':
            ar = '16:9'
        text = (
            ' Crop(%i,%i,-%i,-%i) - %ix%i (%s) - %s  %s \t\t %ix%i (%s)  -  %.03f fps      ' %
            (
    ...
    def GetVideoInfoDict(self, script=None, frame=None, addon=''):
        ...
        h, m = divmod(m, 60)
        time = '%02i:%02i:%02.3f' % (h ,m, s)
        m, s = divmod(framecount/framerate, 60)
        h, m = divmod(m, 60)
        totaltime = '%02i:%02i:%02.3f' % (h ,m, s)
        zoom = ''
        if self.zoomwindow and script.zoomwindow_actualsize is not None:
            ...
            if ID == wx.ID_OK:
                ...
                if ext == '.tag':
                    ...
                    try:
                        ...
                        wx.MessageBox(_('Invalid filter customization file!'), _('Error'), style=wx.ICON_ERROR)
                        f.close()
                        dlg2.Destroy()
                        return
    ...
    def _x_UpdateFunctionDefinitions(self, filterInfo, overwriteAll=False, wrapCalltip=True):
        ...
        if type(filterInfo) != dict:
            wx.MessageBox(_('Invalid argument!'), _('Error'), style=wx.ICON_ERROR)
            return
    ...
    def ShowVideoFrame(self, framenum=None, forceRefresh=False, wrap=True, script=None, userScrolling=False, forceLayout=False, doLayout=True, resize=None, focus=True):
        ...
        if self.UpdateScriptAVI(script, forceRefresh) is None:
            #~ wx.MessageBox(_('Error loading the script'), _('Error'), style=wx.ICON_ERROR)
            return False
    ...
    def PaintAVIFrame(self, inputdc, script, frame, shift=True):
        if script.AVI is None:
            if __debug__:
    ...
    def RunExternalPlayer(self, path=None, script=None, args=None, prompt=True):
        ...
        if not os.path.isfile(path):
            if path != '':
                wx.MessageBox(_('A program must be specified to use this feature!'), _('Error'), style=wx.ICON_ERROR)
            return
    ...
    def createAutoUserSliders(self, script):
        ...
        if row == 0:
            ...
            if script.sliderTexts != []:
                height = 20
            script.sliderSizerNew.Add((0,height), (row,0), wx.DefaultSpan, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 0)
    ...
    def addAvsColorPicker(self, script, argname, value, defaultValue, row, separator, filterName, argIndex):
        ...
        try:
            ...
            defaultColor = wx.Color(r, g, b)
        except:
            defaultColor = wx.Color()
        try:
            r = int(value[0:2],16)
            g = int(value[2:4],16)
            b = int(value[4:6],16)
            ...
            r=g=b=0
        colorButton = colourselect.ColourSelect(parent, wx.ID_ANY, colour=wx.Color(r,g,b), size=(50,23))
    ...
    def MacroExecuteMenuCommand(self, text):
        ...
        if text == '':
            return False
        for item in self.optionsShortcuts:
    ...
    def MacroGetFilename(self, title=_('Open a script or source'), filefilter=None):
        ...
        dlg = wx.FileDialog(self, title,
            self.options['recentdir'], '', filefilter, wx.OPEN)
        ID = dlg.ShowModal()
        if ID == wx.ID_OK:
            filename = dlg.GetPath()
            dirname = os.path.dirname(filename)
    ...
    def MacroGetSaveFilename(self, title=_('Save as')):
        filefilter = _('All files (*.*)|*.*')
        dlg = wx.FileDialog(self, title,
    ...
    def MacroMsgBox(self, txt, title=''):
        if title == _('Error'):
            wx.MessageBox(txt, title, style=wx.ICON_ERROR)
        else:
    ...
    def ExecuteMacro(self, macrofilename='', return_env=False):
        ...
        if os.path.isfile(macrofilename):
            ...
            wx.MessageBox(_("Couldn't find %(macrofilename)s") % locals(), _('Error'), style=wx.ICON_ERROR)
